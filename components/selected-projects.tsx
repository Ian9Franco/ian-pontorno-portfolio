    ...
    <section id="selected-projects" className="py-10 px-6">
      <div className="max-w-4xl mx-auto">
        ...
        <motion.div
          ...
          viewport={{ once: true }}
        >
          <h2 id="selected-projects-heading" className="text-sm font-medium text-white">
            {t.selectedProjects.title}
          </h2>
          {t.selectedProjects.description && (
            <p className="mt-1.5 text-xs text-neutral-400 max-w-2xl">
              {t.selectedProjects.description}
            </p>
          )}
        </motion.div>

        {/* Featured Projects (Compact Card Presentation) */}
        <div className="space-y-4">
          {featuredProjects.map((project, index) => (
            <motion.article
              key={project.id}
              className="rounded-xl p-4 sm:p-5 bg-white/[0.02] border border-white/[0.08] hover:border-white/[0.16] transition-colors"
              initial={{ opacity: 0, y: 16 }}
              whileInView={{ opacity: 1, y: 0 }}
            ...
            >
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    {project.caseStudyUrl ? (
                      <Link
                        ...
                        className="font-medium text-white text-base hover:text-indigo-400 transition-colors inline-flex items-center gap-1.5"
                      >
                        <span>{project.name}</span>
                      </Link>
                    ) : (
                      <a
                        href={project.url || project.readme}
                        target="_blank"
              ...
              {/* Description */}
              <p className="mt-2.5 text-xs sm:text-sm text-gray-300 leading-relaxed text-shadow-sm">
                {project.description}
              </p>
        ...
        {/* More Work (Recovered Pre-PR List Styles) */}
        {otherProjects.length > 0 && (
          <div className="mt-12">
            <h3 className="text-sm font-medium text-white mb-6 text-left">
              {t.selectedProjects.moreTitle || (language === "es" ? "Otros proyectos" : "More work")}
            </h3>
            <div className="space-y-6">
              {otherProjects.map((project, index) => (
                <motion.div
                  key={project.id}
                  className="space-y-3 group rounded-xl p-4 -mx-4 transition-all duration-300 hover:bg-white/[0.02] border border-transparent hover:border-white/[0.06]"
                  initial={{ opacity: 0, y: 16 }}
                  whileInView={{ opacity: 1, y: 0 }}
                ...
                >
                  ...
                  <div className="flex items-start gap-3.5">
                    <div
                      className="w-8 h-8 rounded-lg flex-shrink-0 flex items-center justify-center overflow-hidden mt-0.5 shadow-md"
                      style={{ backgroundColor: projectColors[project.id] || "#888" }}
                    >
                      <Image
                        src={projectIcons[project.id] || "/placeholder-logo.png"}
                        alt={`${project.name} icon`}
                        width={logoSizes[project.id] || 28}
                        height={logoSizes[project.id] || 28}
                        className="w-full h-full object-cover rounded-lg"
                      />
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex flex-wrap items-center gap-2">
                        <a
                          ...
                          target="_blank"
                          rel="noopener noreferrer"
                          className="font-medium text-white text-sm hover:text-gray-300 transition-colors text-shadow-sm"
                        >
                          {project.name}
                        </a>
                  ...
                  {/* Description */}
                  <p className="text-xs sm:text-sm text-gray-300 leading-relaxed text-shadow-sm">
                    {project.description}
                  </p>
                  ...
                  <div className="flex flex-wrap items-center gap-2.5 pt-1">
                    {project.url && (
                      <a
                        ...
                        className="inline-flex items-center gap-1.5 text-xs font-medium px-3 py-1.5 rounded-lg bg-white/[0.05] hover:bg-white/[0.1] text-gray-200 border border-white/[0.1] transition-all"
                      >
                        <span>{language === "es" ? "Visitar Web ↗" : "Live Demo ↗"}</span>
                      </a>
                    )}
